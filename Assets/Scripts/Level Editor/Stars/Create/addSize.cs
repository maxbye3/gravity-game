...
public class addSize : MonoBehaviour
{
  ...
  void Update()
  {
    ...
    GameObject.FindGameObjectWithTag("Create Star Functionality").GetComponent<starCreationMenu>().outsideLimits(size, "size");
    // Can't be 0
    GameObject.FindGameObjectWithTag("Create Star Functionality").GetComponent<starCreationMenu>().nonZero(size);
